...
const out = path.resolve('deliverables/ecommerce-mobile-check');
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--disable-gpu','--no-sandbox','--disable-extensions','--no-first-run','--no-default-browser-check','--remote-debugging-port=9382',`--user-data-dir=${out}/visual-profile`,'about:blank'], {windowsHide:true,stdio:'ignore'});
(async()=>{
let ws;
try {
 let tabs;
 for(let i=0;i<40;i++){try{tabs=await (await fetch('http://127.0.0.1:9382/json')).json();break;}catch{await new Promise(r=>setTimeout(r,250));}}
 ...
 const results=[];
 for(const [width,height] of [[390,844],[768,1024],[844,390],[900,700],[1024,768]]) {
  await call('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:width<901});
  await call('Page.navigate',{url:pathToFileURL(path.resolve('ecommerce-development-landing.html')).href});
  await new Promise(r=>setTimeout(r,900));
  await evaluate('document.fonts.ready.then(()=>true)');
  await evaluate('Promise.all([...document.images].map(i=>{i.loading="eager";return i.decode().catch(()=>{});})).then(()=>true)');
  await evaluate('document.querySelector(".ecommerce-faq summary").focus()');
  ...
  const faqKeyboardOpens=await evaluate('document.querySelector(".ecommerce-faq details").open');
  const faqClicksToggleAll=await evaluate(`[...document.querySelectorAll('.ecommerce-faq details')].every(d=>{d.open=false;d.querySelector('summary').click();const opened=d.open;d.querySelector('summary').click();return opened&&!d.open;})`);
  await evaluate('document.querySelectorAll(".ecommerce-faq details").forEach(d=>d.open=true); window.scrollTo(0,0)');
  await evaluate('document.querySelectorAll(".ecommerce-faq details").forEach(d=>d.open=false); document.activeElement.blur()');
  const layout=await evaluate(`(()=>{ const els=[...document.querySelectorAll('main *,header.site-header *,footer *')]; return {heroBottom:document.querySelector(".ecommerce-hero").getBoundingClientRect().bottom,viewportHeight:innerHeight,viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth,overflow:els.filter(e=>{let r=e.getBoundingClientRect();return r.width>0&&(r.right>innerWidth+1||r.left< -1)&&getComputedStyle(e).visibility!=='hidden'&&!e.closest('[inert]');}).map(e=>({tag:e.tagName,class:e.className,text:e.textContent.trim().slice(0,60),width:e.getBoundingClientRect().width})),clippedText:els.filter(e=>e.matches('h1,h2,h3,p,a,li')&&e.clientWidth>0&&e.scrollWidth>e.clientWidth+2&&!e.closest('[inert]')).map(e=>({tag:e.tagName,class:e.className,text:e.textContent.trim().slice(0,70)})),brokenImages:[...document.images].filter(i=>!i.naturalWidth).map(i=>i.src)};})()`);
  ...
  for(const [i,selector] of (width===390||width===768 ? ['.ecommerce-moments','.ecommerce-support'] : []).entries()) {
  ...
  layout.allFaqsOpenOverflow=await evaluate('document.querySelectorAll(".ecommerce-faq details").forEach(d=>d.open=true); document.documentElement.scrollWidth>innerWidth');
