...
const out = path.resolve('deliverables/media-mobile-check');
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--disable-gpu','--no-sandbox','--disable-extensions','--no-first-run','--no-default-browser-check','--remote-debugging-port=9350',`--user-data-dir=${out}/visual-profile`,'about:blank'], {windowsHide:true,stdio:'ignore'});
(async()=>{
let ws;
try {
 let tabs;
 for(let i=0;i<40;i++){try{tabs=await (await fetch('http://127.0.0.1:9350/json')).json();break;}catch{await new Promise(r=>setTimeout(r,250));}}
 ...
 for(const width of [320,375,390,430]) {
  ...
  for(const [i,selector] of (width===375?['.media-hero','.media-work','.media-services','.media-process','.media-connected','.media-measurement','.media-team','.site-footer']:[]).entries()) {
  ...
  layout.workLinkWorks=await evaluate('location.hash==="#media-selected-work"');
  layout.measurementStacks=await evaluate('getComputedStyle(document.querySelector(".media-measurement__table td")).display==="block"');
