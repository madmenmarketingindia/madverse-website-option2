...
const out = path.resolve('deliverables/media-hero-check');
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--disable-gpu','--no-sandbox','--disable-extensions','--no-first-run','--no-default-browser-check','--remote-debugging-port=9355',`--user-data-dir=${out}/visual-profile`,'about:blank'], {windowsHide:true,stdio:'ignore'});
(async()=>{
let ws;
try {
 let tabs;
 for(let i=0;i<40;i++){try{tabs=await (await fetch('http://127.0.0.1:9355/json')).json();break;}catch{await new Promise(r=>setTimeout(r,250));}}
 ...
 for(const width of [320,390,768,1024,1366,1920]) {
  ...
  for(const [i,selector] of (width===390||width===1366?['.media-hero','.media-work','.media-services','.media-process','.media-connected','.media-measurement','.media-team','.media-faq','#media-contact']:[]).entries()) {
  ...
  await evaluate('document.querySelector(".media-faq summary").focus()');
  layout.focused=await evaluate('document.activeElement.tagName');
  ...
  layout.faqKeyboardOpens=await evaluate('document.querySelector(".media-faq details").open');
  await evaluate('document.querySelector(".media-faq summary").click()');
  layout.faqCloses=await evaluate('!document.querySelector(".media-faq details").open');
  layout.allFaqsToggle=await evaluate('[...document.querySelectorAll(".media-faq details")].every(d=>{d.open=false;d.querySelector("summary").click();const opened=d.open;d.querySelector("summary").click();return opened&&!d.open;})');
  await evaluate('document.querySelector(".media-hero__work").click()');
  layout.workAnchorWorks=await evaluate('location.hash==="#media-selected-work"');
