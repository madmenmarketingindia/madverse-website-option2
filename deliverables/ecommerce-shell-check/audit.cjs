...
const out = path.resolve('deliverables/ecommerce-shell-check');
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--disable-gpu','--no-sandbox','--disable-extensions','--no-first-run','--no-default-browser-check','--remote-debugging-port=9380',`--user-data-dir=${out}/visual-profile`,'about:blank'], {windowsHide:true,stdio:'ignore'});
(async()=>{
let ws;
try {
 let tabs;
 for(let i=0;i<40;i++){try{tabs=await (await fetch('http://127.0.0.1:9380/json')).json();break;}catch{await new Promise(r=>setTimeout(r,250));}}
 ...
 const results=[];
 for(const [width,height] of [[320,844],[390,844],[768,844],[1366,768]]) {
  ...
  for(const [i,selector] of ['#ecommerce-contact'].entries()) {
