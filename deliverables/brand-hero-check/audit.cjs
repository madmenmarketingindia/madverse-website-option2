...
const out = path.resolve('deliverables/brand-hero-check');
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--disable-gpu','--no-sandbox','--disable-extensions','--no-first-run','--no-default-browser-check','--remote-debugging-port=9366',`--user-data-dir=${out}/visual-profile`,'about:blank'], {windowsHide:true,stdio:'ignore'});
(async()=>{
let ws;
try {
 let tabs;
 for(let i=0;i<40;i++){try{tabs=await (await fetch('http://127.0.0.1:9366/json')).json();break;}catch{await new Promise(r=>setTimeout(r,250));}}
 ...
 for(const width of [320,768,1366]) {
  ...
  for(const [i,selector] of ['#brand-strategy-contact'].entries()) {
